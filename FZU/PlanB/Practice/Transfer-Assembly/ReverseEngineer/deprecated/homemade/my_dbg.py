...
GetModuleHandleA = k32.GetModuleHandleA
...
GetModuleFileNameA = k32.GetModuleFileNameA
...
CreateProcessA = k32.CreateProcessA
...
SetThreadContext = k32.SetThreadContext
...
GetThreadContext = k32.GetThreadContext
...
class Debugger:
    ...
    def read_process_mem(self, addr, length):
        ...
        count = c_ulonglong(0)
        if not ReadProcessMemory(self.h_process, addr, read_buf, length, byref(count)):
            return None
        else:
            data += read_buf.raw
            return data
    ...
    def write_process_mem(self, addr, data):
        ...
        if not WriteProcessMemory(self.h_process, addr, c_data, length, byref(count)):
            return False
        else:
            return True
        ...
        if addr not in self.breakpoints.keys():
            try:
                ...
                if not self.write_process_mem(addr=addr, data=b"\xCC"):
                    print('Cannot write process memory, err code:', GetLastError())
                    return False
                else:
    ...
    def resolve_dll_func(dll_name, func_name):
        handle = GetModuleHandleA(dll_name.encode('utf-8'))
        addr = GetProcAddress(handle, func_name.encode('utf-8'))
        CloseHandle(handle)
    ...
    def get_debug_event(self):
        ...
        if WaitForDebugEvent(byref(debug_event), INFINITE):
    ...
    def exception_handler_breakpoint(self):
        ...
        if self.exception_addr in self.breakpoints.keys():
            ...
            SetThreadContext(self.h_thread, byref(self.context))
        return DBG_CONTINUE
    ...
    @staticmethod
    def open_thread(tid):
        h_thread = OpenThread(THREAD_ALL_ACCESS, None, tid)
        if h_thread is None:
    ...
    def enum_threads(self):
        ...
        if snapshot is not None:
            ...
            success = Thread32First(snapshot, byref(thread_entry))
            while success:
                if thread_entry.th32OwnerProcessID == self.pid:
                    threads_list.append(thread_entry.th32ThreadID)
                success = Thread32Next(snapshot, byref(thread_entry))
    ...
    def get_thread_ctx(self, tid=None, h_thread=None):
        ...
        if GetThreadContext(h_thread, byref(ctx)):
            return ctx
        else:
