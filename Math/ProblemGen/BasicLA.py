...
class LinearAlgebraProblemGenerator:
    ...
    def _generate_equation_problem(self):
            ...
            for j in range(n):
                ...
                if coeff != 0:
                    ...
                    if coeff == 1:
                        ...
                        term = f"{coeff}{variables[j]}"
                    terms.append(term)

            # 构建方程字符串
            if not terms:  # 所有系数都为0
                equation = "0"
            else:
                # 第一项直接使用，后续项带符号
                equation = terms[0]
        ...
        # 验证显示与求解的一致性
        problem += f"\n系数矩阵A:\n{self._matrix_to_str(a)}"
        problem += f"常数向量b:\n{self._matrix_to_str(b)}"
        # 计算解
    ...
    @staticmethod
    def _generate_integer_matrix(rows, cols, min_val, max_val):
        """生成整数矩阵"""
        matrix = []
        for _ in range(rows):
    ...
    @staticmethod
    def _matrix_to_str(matrix, decimal=False):
        """将矩阵转换为字符串表示"""
        result = ""
        for row in matrix:
        ...
        while True:
            ...
            if choice in self.problem_types:
                ...
                if choice == '1':
                    ...
                    raise ValueError('wrong choice:{}'.format(choice))
                print("\n【题目】")
                print(problem)
