...
def console_login(params):
    ...
    try:
        ...
        if console_type == 'serial':
            ...
            ser = serial.Serial(port=console_serial_port, baudrate=console_serial_baud_rate, timeout=0.5)
            if 'serial_type' in console_login_params and console_login_params['serial_type'] == 'RJ':
                ...
                while not finished:
                    ...
                    print(echo_string)
                    if 'root@Ruijie' in echo_string or 'root@Reyee' in echo_string:
                        finished = True
                        ...
                        ser.write(data='{}\r'.format(console_serial_pass).encode('utf-8'))
                    else:
            ...
            ssh_.connect(console_ssh_ip, console_ssh_port, username=console_ssh_user, password=console_ssh_pass)
            ssh_shell = ssh_.invoke_shell()
            console_login_params.__setitem__('ssh', ssh_shell)
        elif console_type == 'telnet':
            raise NotImplementedError('telnet console has not been implemented yet.')
        else:
...
def console_iwpriv_site_survey(params):
    ap_cli = params['console']['ap_cli']
    params['console']['send_string'] = 'iwpriv {} set sitesurvey=1'.format(ap_cli)
    params['console']['format'] = 'str'
    params['console']['wait'] = 5
    params = console_send(params=params)
    params['console']['send_string'] = 'iwpriv {} get_site_survey'.format(ap_cli)
    params['console']['format'] = 'str'
    params['console']['wait'] = 5
    params = console_send(params=params)
    ...
    params['console']['send_string'] = 'iwpriv {} set sitesurvey=0'.format(ap_cli)
    params['console']['format'] = 'str'
    params = console_send(params=params)
    return params
...
def console_iwpriv_reg(params):
    ...
    params['console']['send_string'] = 'iwpriv {} mac {}'.format(ap_cli, iwpriv_mac)
    params['console']['format'] = 'str'
    params['console']['wait'] = 5
    params = console_send(params=params)
    iwpriv_text = params['console']['echo_string']
