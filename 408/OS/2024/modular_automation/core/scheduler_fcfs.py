...
def api_process_queued_task():
    while True:
        ...
        rt_lock.acquire()
        std_check_waiting_task()
        std_remove_finished_task()
        rt_lock.release()
        time.sleep(0.25)
...
def std_check_conflict_task(runnable, task):
...
def std_check_waiting_task():
    if len(running_tasks.keys()) < workers_count:
        if len(waiting_tasks.keys()) > 0:
            ...
            runnable = True
            runnable = std_check_conflict_task(runnable, task)
            if runnable:
                std_execute_runnable_task(task=task)
            else:
                api_create_waiting_task(task=task)


def std_execute_runnable_task(task):
    task_id = task[0]
