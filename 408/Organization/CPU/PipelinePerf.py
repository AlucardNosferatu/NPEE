...
class PipelinePerformanceAnalyzer:
    ...
    def calculate_performance(problem):
        ...
        # 修复：第一条指令的总耗时应该是段数×时钟周期，而不是各段时间之和
        first_instruction_time = num_stages * clock_cycle
        theoretical_time = first_instruction_time + (num_instructions - 1) * clock_cycle
...
"""
    ...
    def format_solution(problem, solution):
        ...
        num_instructions = problem["num_instructions"]
        hazard_info = problem["hazard_info"]
        clock_cycle = solution['clock_cycle']

        solution_text = f"""
解答：

1. 流水线的时钟周期：
   时钟周期等于各段执行时间的最大值，即 max({', '.join(map(str, stage_times))}) = {clock_cycle}ns

2. 不考虑数据冲突的总时间：
   总时间 = 第一条指令耗时 + (n - 1) × 时钟周期
          = {problem["num_stages"]} × {clock_cycle} + ({num_instructions} - 1) × {clock_cycle}
