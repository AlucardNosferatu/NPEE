...
class TransportLayerQuizGenerator:
    ...
    def _generate_throughput_question(self):
        ...
        bandwidth = random.randint(*self.bandwidth_range)
        window_size = random.randint(*self.window_size_range)
        frame_size = random.randint(*self.frame_size_range)

        # 计算正确答案（单位：Mbps）
        # 吞吐量 = (窗口大小 * 帧大小 * 8) / (RTT / 1000) / (1000 * 1000)
        answer = (window_size * frame_size * 8) / (rtt / 1000) / (1000 * 1000)

        question = f"已知某网络的RTT为{rtt}ms，带宽为{bandwidth}Mbps，窗口大小为{window_size}帧，帧大小为{frame_size}字节。请计算其最大吞吐量（单位：Mbps）。"
        explanation = f"吞吐量计算公式为：(窗口大小 × 帧大小 × 8) / RTT = ({window_size} × {frame_size} × 8) / {rtt} = {answer:.2f}Mbps"
